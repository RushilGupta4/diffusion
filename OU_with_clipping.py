...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=total_steps, desc=f"Epoch {epoch+1}/{num_epochs}") as pbar:
            for _ in range(T):
                for (x0,) in dataloader:
                    ...
                    t_norm = t.float() / T * MAX_T
                    t_norm = t_norm.unsqueeze(1)

                    noise = torch.randn_like(x0)
                    std = torch.sqrt(1 - torch.exp(-2 * t_norm))
                    x_t = (
                        x0 * torch.exp(-t_norm)
                        + std * noise
                    )
        ...
        print(
            f"Epoch {epoch+1}/{num_epochs}, Loss: {epoch_loss:.6f}, Mean: ({mean_x:.4f}, {mean_y:.4f}), Eval Score: {eval_score:.6f}, Best: {best_loss:.6f}\n\n"
        )


@torch.no_grad()
def sample(model, num_samples, T, MAX_T, device, theta):
    model.eval()
    x = torch.randn(num_samples, 2, device=device)  # x0^R ~ N(0, I)
    dt = MAX_T / T  # Simple discretization step
    ...
    for i in reversed(range(T)):
        ...
        t = torch.full((num_samples,), i / T, device=device) * MAX_T
        ...
        shifted_input = (
            x + (torch.exp(t).unsqueeze(1) - torch.exp(-t).unsqueeze(1)) * theta
        )
        
        pred_noise = model(shifted_input, t)

        eps = 1e-5
        t = torch.clamp(t, min=eps)  # Ensure t is at least eps
        std = torch.sqrt(1 - torch.exp(-2 * t).unsqueeze(1))
        score_component = -pred_noise / std
        
        # Combine explicit and learned components
        score = term1 + score_component
        ...
        x = (
            x
            + (x + score) * dt
            + torch.sqrt(torch.tensor(2 * dt, device=device)) * noise
        )
...
def main():
    TRAIN = False
    step = 2
    ckpt = "modelt1_{step}.pth"

    # Hyperparameters
    MAX_T = 1
    T = MAX_T * 500 # total number of diffusion steps
    ...
    print(
        f"Weighted Average x: {weighted_avg[0]:.4f}, Weighted Average y: {weighted_avg[1]:.4f}"
    )
